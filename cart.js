...
angular
  ...
  .factory(
    ...
    function (
        ...
        if (twoStep) {
          ...
          if (chgModel)
            ...
            urlParameters["query"] =
              ...
              currentVersion;
          var targetQuery =
            additionalParms && additionalParms["target_query"]
              ? additionalParms["target_query"]
              : "";
          var parentSysId =
            additionalParms && additionalParms["sysparm_parent_sys_id"]
              ? additionalParms["sysparm_parent_sys_id"]
              : "";
          var targetQueryFieldsAndValues = parseEncodedQuery(targetQuery);
          if (
            !targetQueryFieldsAndValues.hasOwnProperty("parent") &&
            parentSysId
          )
            urlParameters["query"] += "^parent=" + parentSysId;
          if (targetQuery) urlParameters["query"] += "^" + targetQuery;
          var completeUrl =
      ...
      function getDynamicValue(recordId, triggerField, targetFields) {
        return $http
          .post(
            baseCatalogUrl + "variables/" + triggerField + "/dynamic_value",
            {
              sysparm_dynamic_target_fields: targetFields.toString(),
              sysparm_dynamic_record_id: recordId,
            }
          )
      ...
      function parseEncodedQuery(queryString) {
        var fieldsAndValues = {};
        if (!queryString || !queryString.match(/=/g)) return fieldsAndValues;
        var queryStringElements = queryString
          .replaceAll("^^", "&hat;")
          .split("^");
        queryStringElements.forEach(function (element) {
          if (!element.match(/=/g)) return;
          var fieldAndValue = element.split("=");
          if (!fieldAndValue[0]) return;
          fieldsAndValues[fieldAndValue[0]] = fieldAndValue
            .splice(1)
            .join("=")
            .replaceAll("&hat;", "^^");
        });
